...
// Package login implements the butleradm login command.
package login
...
// NewLoginCmd creates the login command for butleradm.
...
butleradm login requires platform administrator privileges. If the
authenticated user is not a platform admin, login will fail.

Examples:
  # Login to a Butler server
  butleradm login --server https://butler.example.com

  # Login without opening a browser (copy the URL manually)
  butleradm login --server https://butler.example.com --no-browser`,
...
func runLogin(ctx context.Context, logger *log.Logger, serverURL string, noBrowser bool) error {
	...
	// Reject non-platform-admins
	if !tr.User.IsPlatformAdmin {
		fmt.Println()
		return fmt.Errorf("account is not a platform admin. butleradm requires platform administrator privileges")
	}
