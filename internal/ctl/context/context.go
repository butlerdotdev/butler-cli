...
// Package context implements the butlerctl context commands for switching
// team context.
package context
...
Examples:
  ...
  # Switch active team
  butlerctl context use payments`,
	}
...
Examples:
  ...
  butlerctl context list --server https://butler.example.com`,
		Aliases: []string{"ls"},
...
func runList(logger *log.Logger, serverURL string) error {
	...
	if server == "" {
		return fmt.Errorf("no active server; run 'butlerctl login --server URL' first")
	}

	sc, ok := creds.Servers[server]
	if !ok {
		return fmt.Errorf("no credentials for server %s; run 'butlerctl login --server %s' first", server, server)
	}
...
Examples:
  ...
  butlerctl context use payments --server https://butler.example.com`,
